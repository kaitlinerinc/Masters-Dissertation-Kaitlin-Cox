...
expand_with_threshold <- function(seeds, matrix, threshold = 0.80, top_n = 50){
  ...
  stop_words <- c(
    ...
    "attention", "critical", "focus", "focused", "committed"
  )
...
# ==============================================================================
df <- bind_rows(
  prepare_ads(readRDS(NEWSPAPER_INPUT), "Newspaper"),
...
dics <- list(
  victimhood = unique(c(
    "helpless", "vulnerable", "victim", "suffering", "desperate", "weak", 
    "hopeless", "abandoned", "destitute", "impoverished", "dying", "needy", "fragile",
    "hunger", "hungry", "starvation", "thirst", "cold", "sick", "disease", 
    "homeless", "food", "water", "shelter", "famine", "malnutrition"
  )),
  charity_agency = c("help", "save", "provide", "deliver", "aid", "support", "protect", "transform"),
  beneficiary_passivity = c("received", "given", "granted", "accepted", "passive", "beneficiary", "recipient")
)

expanded_dictionaries <- map(dics, ~expand_with_threshold(.x, glove_matrix, 0.80, 50))

# ----------------- Manual Pruning -----------------
victim_noise <- c("enough", "longer", "away", "eventually", "far", "giving", "living", "none", "out", "about", "lack", "supposed", "wonder", "happens", "ever", "except", "mind", "little", "seriously", "turn", "else", "whole", "over", "once", "mention", "taken", "person", "nobody", "seem", "remain", "equally", "completely", "unaware", "powerless", "dealing", "surely", "seemed", "unfortunately", "sadly", "bad", "terrible", "worse", "worst", "difficult")
charity_noise <- c("lastly", "full", "good", "order", "out", "put", "easy", "works", "ready", "advantage", "key", "time", "rely", "required", "additional", "let", "enough", "access", "longer", "safe", "easier", "step", "example", "effort", "opportunity", "choice", "ability", "change", "need", "needs", "needed")
...
df <- df %>%
  mutate(
    ...
    Victimhood_Density     = vic_hits / (total_tokens + 1e-6) * 1000,
    Charity_Agency_Density = char_age_hits / (total_tokens + 1e-6) * 1000,
    Bene_Pass_Density      = bene_pass_hits / (total_tokens + 1e-6) * 1000,
    
    # NEW: Log Ratio of Agency to Passivity
    # +1 Laplace smoothing prevents division by zero. Log centers neutral balance at 0.
    Agency_Passivity_Ratio = log((char_age_hits + 1) / (bene_pass_hits + 1))
  )

analysis_df <- df %>% mutate(log_tokens = log(total_tokens + 1))
news_df <- analysis_df %>% filter(Source_Type == "Newspaper")

# Regressions (4 models now)
m_vic   <- lm(Victimhood_Density ~ Is_International + log_tokens, news_df)
m_char  <- lm(Charity_Agency_Density ~ Is_International + log_tokens, news_df)
m_bene  <- lm(Bene_Pass_Density ~ Is_International + log_tokens, news_df)
m_ratio <- lm(Agency_Passivity_Ratio ~ Is_International + log_tokens, news_df) # NEW MODEL
...
descriptive_stats <- df %>%
  mutate(sentence_count = stri_count_regex(CleanedText, "[.!?]+")) %>%
  summarise(
...
stargazer(m_vic, m_char, m_bene, m_ratio, 
          type = "text", 
          se = robust_se, # Uses the robust standard errors we calculated
          ...
          covariate.labels = c("Is International (Yes=1)", "Log Tokens (Word Count)"),
...
theme_summary <- news_df %>%
  ...
  summarise(
    ...
    Mean_Ratio = mean(Agency_Passivity_Ratio, na.rm = TRUE), # NEW
    SD_Ratio = sd(Agency_Passivity_Ratio, na.rm = TRUE)      # NEW
  )
print(theme_summary)



# Count initial ads before filtering
...
# Calculate and print retention report
retained_count <- nrow(df)
dropped_count  <- initial_count - retained_count

cat("\n--- ADVERTISEMENT RETENTION REPORT ---\n")
cat("Initial advertisements:  ", initial_count, "\n")
cat("Retained advertisements: ", retained_count, "\n")
cat("Dropped advertisements:  ", dropped_count, "\n")
cat("Retention Rate:          ", round((retained_count / initial_count) * 100, 2), "%\n")
cat("---------------------------------------\n")
